...
object Perceptron extends JFXApp {
  ...
  val weights = ObservableBuffer(
    Seq.fill(outputs)(
      Weights(Array.fill(inputs + 1)((math.random - 0.5) * 0.2))
    ): _*
  )
    ...
    scene = new Scene(1200, 1000) {
      ...
      val resultColumns = (0 until outputs).map { i =>
        ...
        col.cellValueFactory = dr =>
          ObjectProperty(
            (1.0 +: dr.value.inputs, weights(i).w).zipped.map(_ * _).sum
          )
        col
      }
      ...
      val learnButton2 = new Button("Learn 100")
      ...
      val vBox = new VBox(dataTable, weightTable, learnButton, learnButton2) //scrollPane1a, scrollPane1b)
  ...
  def learnOne(): Unit = {
    ...
    val ys = (0 until outputs).map(
      i => (1.0 +: learnData(index).inputs, weights(i).w).zipped.map(_ * _).sum
    )
    for (o <- 0 until outputs; i <- 0 until inputs + 1) {
      val augmentedInputs = 1.0 +: learnData(index).inputs
      weights(o).w(i) += eta * (learnData(index).outputs(o) - ys(o)) * augmentedInputs(
        i
      )
    }
  ...
  def drawData(): Unit = {
    ...
    for (i <- 0 until 100; j <- 0 until 100) {
      val x = (i - 50) / 20.0
      val y = (j - 50) / 20.0
      val out = (Array(1.0, x, y), weights(0).w).zipped
        .map(_ * _)
        .sum
